...
BEGIN
  ...
  FOR v_consumo IN SELECT * FROM jsonb_array_elements(p_consumos) LOOP
    ...
    -- Si no hubo suficiente inventario
    IF v_cantidad_requerida > 0 THEN
...
const projectRef = 'schhqtttjysiyghwmefv';
const password = 'Agrifeed.08';
const encodedPassword = encodeURIComponent(password);

async function tryConnect() {
   for (const region of regions) {
     const uri = 'postgresql://postgres:' + encodedPassword + '@db.schhqtttjysiyghwmefv.supabase.co:5432/postgres';
